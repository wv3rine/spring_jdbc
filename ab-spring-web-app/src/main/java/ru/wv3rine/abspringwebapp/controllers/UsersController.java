...
public class UsersController {
    ...
    public List<UserIdAndLogin> getUsersNames(@Min(value = 1, message = "Page size must be greater than one!")
    ...
    @PutMapping(path = "/{id}")
    public User updatePerson(@Valid @NotNull @RequestBody User user,
                             BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            throw new NotEnoughArgumentsException("User must have login and password");
        }
        return usersService.updateUserById(user);
    }
}
